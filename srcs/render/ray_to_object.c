...
t_vector	ray_direction(t_camera *cam, t_image_size s, t_pixel_cdts p)
{
	...
	focal_len = s.W / (2.0f * tanf(((float) cam->fov * ((float) M_PI / 180.0f)) / 2.0f));
	projection_point.x = ((2.0f * (p.x + 0.5f) / s.W) - 1.0f)
		* (s.W / (2.0f * focal_len));
	projection_point.y = (1.0f - (2.0f * (p.y + 0.5f) / s.H))
		* (s.H / (2.0f * focal_len));
	v_up = orthogonal_vector(cam->vector, 1.0f, 1.0f);
